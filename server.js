...
GlobalFonts.registerFromPath(
  join(__dirname, "fonts", "Inter-Medium.ttf"),
  "Inter-Medium"
);
GlobalFonts.registerFromPath(
  join(__dirname, "fonts", "Inter-SemiBold.ttf"),
  "Inter-SemiBold"
);
GlobalFonts.registerFromPath(
...
GlobalFonts.registerFromPath(
  join(__dirname, "fonts", "Inter-ExtraBold.ttf"),
  "Inter-ExtraBold"
);
GlobalFonts.registerFromPath(
...
GlobalFonts.registerFromPath(
  join(__dirname, "fonts", "NotoSansKR-Bold.otf"),
  "NotoSansKR-Bold"
);
GlobalFonts.registerFromPath(
  join(__dirname, "fonts", "NotoSansSC-Bold.otf"),
  "NotoSansSC-Bold"
);

const FONT_FAMILY =
  "Inter, Noto Sans SC, Noto Sans JP, Noto Sans KR, sans-serif";
...
async function drawBackground(ctx, canvas, primaryColor, secondaryColor) {
  ...
  ctx.fillRect(
    0,
    canvas.height - BACKGROUND_BORDER_SIZE,
    canvas.width,
    BACKGROUND_BORDER_SIZE
  );
  ctx.fillRect(
    canvas.width - BACKGROUND_BORDER_SIZE,
    0,
    BACKGROUND_BORDER_SIZE,
    canvas.height
  );
  ...
  ctx.fillRect(
    0,
    BACKGROUND_BOTTOM_BAR_TL_Y,
    canvas.width,
    BACKGROUND_BOTTOM_BAR_HEIGHT
  );
}
...
async function drawCharacterSkills(
  ctx,
  characterSkillsData,
  primaryColor,
  secondaryColor
  ...
  for (let i = 0; i < 4; i++) {
    ...
    // Picture from URL
    const skillIconURL =
      characterSkillsData[`character_${skillText[i]}_icon_url`];
    const skillIcon = await colorImage(
      `${ASSETS_BASE_URL}${skillIconURL}`,
      secondaryColor
    );
    ctx.drawImage(
      skillIcon,
      SKILL_ITEM_TL_X + SKILL_WIDTH_PADDING * i,
      SKILL_ITEM_TL_Y,
      SKILL_ITEM_DIMENSIONS,
      SKILL_ITEM_DIMENSIONS
    );
    // Skill tag
    ctx.fillStyle = secondaryColor;
    ctx.fillRect(
      SKILL_TAG_TL_X + SKILL_WIDTH_PADDING * i,
      SKILL_TAG_TL_Y,
      SKILL_TAG_WIDTH,
      SKILL_TAG_HEIGHT
    );
    ...
    ctx.fillText(
      `${skillText[i]}`,
      SKILL_DESC_TL_X + SKILL_WIDTH_PADDING * i,
      SKILL_DESC_TL_Y
    );
    // Skill tag text
    ...
    const skillLevel = prependZero(
      characterSkillsData[`character_${skillText[i]}_level`]
    );
    ctx.fillText(
      `${skillLevel}`,
      SKILL_TAG_TEXT_TL_X + SKILL_WIDTH_PADDING * i,
      SKILL_TAG_TEXT_TL_Y
    );
  }
  ctx.textAlign = "left";
...
async function drawCharacterWindow(
  ctx,
  characterData,
  primaryColor,
  secondaryColor
  ...
  ctx.fillStyle = secondaryColor;
  ctx.fillRect(
    CHARACTER_ART_TL_X - 4,
    CHARACTER_ART_TL_Y - 4,
    CHARACTER_ART_WIDTH + 8,
    CHARACTER_ART_HEIGHT + 8
  );
  // Character picture
  const characterPicture = await cropImage(
    characterData.character_icon_url,
    CHARACTER_ART_WIDTH,
    CHARACTER_ART_HEIGHT,
    CHARACTER_ART_SCALE_RATIO,
    CHARACTER_ART_CROP_X_OFFSET,
    CHARACTER_ART_CROP_Y_OFFSET
  );
  ctx.drawImage(
    characterPicture,
    CHARACTER_ART_TL_X,
  ...
  ctx.drawImage(
    await colorImage(
      `${ASSETS_BASE_URL}${characterData.character_path_icon_url}`,
      primaryColor
    ),
    CHARACTER_PATH_TL_X,
    CHARACTER_PATH_TL_Y,
    CHARACTER_PATH_DIMENSIONS,
    CHARACTER_PATH_DIMENSIONS
  );
  ctx.drawImage(
    await colorImage(
      `${ASSETS_BASE_URL}${characterData.character_element_icon_url}`,
      primaryColor
    ),
    CHARACTER_ELEMENT_TL_X,
    CHARACTER_ELEMENT_TL_Y,
    CHARACTER_ELEMENT_DIMENSIONS,
    CHARACTER_ELEMENT_DIMENSIONS
  );
  // Character name
  ...
  ctx.textAlign = "right";
  // Remove HTML tags
  const characterName = characterData.character_name.replace(
    /<\/?[^>]+(>|$)/g,
    ""
  );
  ...
  ctx.fillRect(
    CHARACTER_EIDOLON_TAG_TL_X,
    CHARACTER_EIDOLON_TAG_TL_Y,
    CHARACTER_EIDOLON_TAG_WIDTH,
    CHARACTER_EIDOLON_TAG_HEIGHT
  );
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    `E${characterData.character_rank}`,
    CHARACTER_EIDOLON_TAG_TEXT_TL_X,
    CHARACTER_EIDOLON_TAG_TEXT_TL_Y
  );
  ctx.textAlign = "left";
  // Character level highlight box
  ctx.fillStyle = secondaryColor;
  ctx.fillRect(
    CHARACTER_LEVEL_BAR_TL_X,
    CHARACTER_LEVEL_BAR_TL_Y,
    CHARACTER_LEVEL_BAR_WIDTH,
    CHARACTER_LEVEL_BAR_HEIGHT
  );
  ...
  ctx.fillText(
    ...
    CHARACTER_LEVEL_VALUE_TL_X,
    CHARACTER_LEVEL_VALUE_TL_Y
  );

  ctx.textAlign = "left";
}

async function drawCharacterStats(
  ctx,
  characterAttributesData,
  characterAdditionsData,
  secondaryColor
) {
  ...
  ctx.fillText(
    "ATK",
    CHARACTER_STAT_NAME_TL_X,
    CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT
  );
  ctx.fillText(
    "DEF",
    CHARACTER_STAT_NAME_TL_X,
    CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT * 2
  );
  ctx.fillText(
    "SPD",
    CHARACTER_STAT_NAME_TL_X,
    CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT * 3
  );
  ctx.fillText(
    "CRIT Rate",
    CHARACTER_STAT_NAME_TL_X,
    CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT * 4
  );
  ctx.fillText(
    "CRIT DMG",
    CHARACTER_STAT_NAME_TL_X,
    CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT * 5
  );

  // Character base stat values (right aligned)
  ctx.textAlign = "right";
  // Add base values with character additions if it exists
  const baseHP = addBaseValue(
    characterAdditionsData,
    characterAttributesData.attributes_base_hp,
    "HP"
  );
  const baseATK = addBaseValue(
    characterAdditionsData,
    characterAttributesData.attributes_base_atk,
    "ATK"
  );
  const baseDEF = addBaseValue(
    characterAdditionsData,
    characterAttributesData.attributes_base_def,
    "DEF"
  );
  const baseSPD = addBaseValue(
    characterAdditionsData,
    characterAttributesData.attributes_base_spd,
    "SPD"
  );
  const baseCritRate = addBaseValueFloat(
    characterAdditionsData,
    characterAttributesData.attributes_base_crit_rate,
    "CRIT Rate"
  );
  const baseCritDMG = addBaseValueFloat(
    characterAdditionsData,
    characterAttributesData.attributes_base_crit_dmg,
    "CRIT DMG"
  );
  ...
  for (const [index, stat] of baseStats.entries()) {
    ...
    ctx.fillText(
      ...
      CHARACTER_STAT_VALUE_TL_Y + CHARACTER_STAT_LINE_HEIGHT * index
    );
    // Draw stat icon
    const baseStatIconURL = `attributes_${baseStatsName[index]}_icon`;
    const baseStatIcon = await colorImage(
      `${ASSETS_BASE_URL}${characterAttributesData[baseStatIconURL]}`,
      secondaryColor
    );
    ctx.drawImage(
      baseStatIcon,
      CHARACTER_STAT_ICON_TL_X,
      CHARACTER_STAT_ICON_TL_Y + CHARACTER_STAT_LINE_HEIGHT * index,
      CHARACTER_STAT_ICON_DIMENSIONS,
      CHARACTER_STAT_ICON_DIMENSIONS
    );
  }
  ...
  for (const [_, value] of Object.entries(characterAdditionsData)) {
    ...
    ctx.fillText(
      `${value.name}`,
      CHARACTER_STAT_NAME_TL_X,
      CHARACTER_STAT_NAME_TL_Y + CHARACTER_STAT_LINE_HEIGHT * (6 + i)
    );
    ctx.textAlign = "right";
    ...
    const statIcon = await colorImage(
      `${ASSETS_BASE_URL}${value.icon}`,
      secondaryColor
    );
    ctx.drawImage(
      statIcon,
      CHARACTER_STAT_ICON_TL_X,
      CHARACTER_STAT_ICON_TL_Y + CHARACTER_STAT_LINE_HEIGHT * (6 + i),
      CHARACTER_STAT_ICON_DIMENSIONS,
      CHARACTER_STAT_ICON_DIMENSIONS
    );
    i++;
...
async function drawRelics(
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ctx.fillRect(
      RELIC_LEFT_ITEM_TL_X - 4,
      RELIC_LEFT_ITEM_TL_Y - 4 + i * RELIC_HEIGHT_PADDING,
      RELIC_ITEM_DIMENSIONS + 8,
      RELIC_ITEM_DIMENSIONS + 8
    );
    // Relic item box
    ctx.fillStyle = primaryColor;
    ctx.fillRect(
      RELIC_LEFT_ITEM_TL_X,
      RELIC_LEFT_ITEM_TL_Y + i * RELIC_HEIGHT_PADDING,
      RELIC_ITEM_DIMENSIONS,
      RELIC_ITEM_DIMENSIONS
    );
    try {
      // Picture from URL
      const relicImage = await loadImage(
        `${ASSETS_BASE_URL}${relicsData[i].icon_url}`
      );
      // Resize to 96px
      ctx.drawImage(
        relicImage,
        RELIC_LEFT_ITEM_TL_X + 16,
        RELIC_LEFT_ITEM_TL_Y + 16 + i * RELIC_HEIGHT_PADDING,
        96,
        96
      );
      // Relic box tag
      ...
      ctx.fillRect(
        RELIC_LEFT_TAG_TL_X,
        RELIC_LEFT_TAG_TL_Y + i * RELIC_HEIGHT_PADDING,
        RELIC_TAG_WIDTH,
        RELIC_TAG_HEIGHT
      );
      // Draw relic stats
      await drawRelicStats(
        ctx,
        relicsData[i],
        i,
        "left",
  ...
  for (let i = 1; i < 3; i++) {
    ...
    ctx.strokeStyle = secondaryColor;
    ctx.fillRect(
      RELIC_RIGHT_ITEM_TL_X - 4,
      RELIC_RIGHT_ITEM_TL_Y - 4 + i * RELIC_HEIGHT_PADDING,
      RELIC_ITEM_DIMENSIONS + 8,
      RELIC_ITEM_DIMENSIONS + 8
    );
    // Relic item box
    ctx.fillStyle = primaryColor;
    ctx.fillRect(
      RELIC_RIGHT_ITEM_TL_X,
      RELIC_RIGHT_ITEM_TL_Y + i * RELIC_HEIGHT_PADDING,
      128,
      128
    );
    ...
    try {
      // Picture from URL
      const relicImage = await loadImage(
        `${ASSETS_BASE_URL}${relicsData[i + 3].icon_url}`
      );
      // Resize to 96px
      ctx.drawImage(
        relicImage,
        RELIC_RIGHT_ITEM_TL_X + 16,
        RELIC_RIGHT_ITEM_TL_Y + 16 + i * RELIC_HEIGHT_PADDING,
        96,
        96
      );
      // Relic box tag
      ctx.fillStyle = secondaryColor;
      ctx.fillRect(
        RELIC_RIGHT_TAG_TL_X,
        RELIC_RIGHT_TAG_TL_Y + i * RELIC_HEIGHT_PADDING,
        RELIC_TAG_WIDTH,
        RELIC_TAG_HEIGHT
      );
      // Draw relic stats
...
async function drawRelicStats(
  ...
  try {
    if (side === "left") {
      ...
      ctx.fillStyle = secondaryColor;
      ctx.fillRect(
        RELIC_LEFT_STAT_BAR_TL_X,
        RELIC_LEFT_STAT_BAR_TL_Y + i * RELIC_HEIGHT_PADDING,
        RELIC_STAT_BAR_WIDTH,
        RELIC_STAT_BAR_HEIGHT
      );
      ...
      ctx.fillText(
        `+${relicLevel}`,
        RELIC_LEFT_TAG_TEXT_TL_X,
        RELIC_LEFT_TAG_TEXT_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      ...
      ctx.textAlign = "left";
      ctx.fillText(
        `${relicData.main_affix.name}`,
        RELIC_LEFT_STAT_NAME_TL_X,
        RELIC_LEFT_STAT_NAME_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      // Relic main stat value
      ctx.textAlign = "right";
      ctx.fillText(
        `${relicData.main_affix.display}`,
        RELIC_LEFT_STAT_VALUE_TL_X,
        RELIC_LEFT_STAT_VALUE_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      // Draw every sub stat
      ctx.font = `${FONT_SEMIBOLD} ${FONT_SIZE_SMALLEST} ${FONT_FAMILY}`;
      ctx.fillStyle = secondaryColor;
      for (let j = 0; j < relicData.sub_affix.length; j++) {
        ctx.textAlign = "left";
        ctx.fillText(
          `${relicData.sub_affix[j].name}`,
          RELIC_LEFT_STAT_NAME_TL_X,
          RELIC_LEFT_STAT_NAME_TL_Y +
            i * RELIC_HEIGHT_PADDING +
            RELIC_STAT_LINE_HEIGHT * (j + 1)
        );
        ctx.textAlign = "right";
        ctx.fillText(
          `${relicData.sub_affix[j].display}`,
          RELIC_LEFT_STAT_VALUE_TL_X,
          RELIC_LEFT_STAT_VALUE_TL_Y +
            i * RELIC_HEIGHT_PADDING +
            RELIC_STAT_LINE_HEIGHT * (j + 1)
        );
      }
    ...
    if (side === "right") {
      ...
      ctx.fillStyle = secondaryColor;
      ctx.fillRect(
        RELIC_RIGHT_STAT_BAR_TL_X,
        RELIC_RIGHT_STAT_BAR_TL_Y + i * RELIC_HEIGHT_PADDING,
        RELIC_STAT_BAR_WIDTH,
        RELIC_STAT_BAR_HEIGHT
      );
      ...
      ctx.fillText(
        `+${relicLevel}`,
        RELIC_RIGHT_TAG_TEXT_TL_X,
        RELIC_RIGHT_TAG_TEXT_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      ...
      ctx.fillText(
        `${relicData.main_affix.name}`,
        RELIC_RIGHT_STAT_NAME_TL_X,
        RELIC_RIGHT_STAT_NAME_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      // Relic main stat value
      ctx.textAlign = "right";
      ctx.fillText(
        `${relicData.main_affix.display}`,
        RELIC_RIGHT_STAT_VALUE_TL_X,
        RELIC_RIGHT_STAT_VALUE_TL_Y + i * RELIC_HEIGHT_PADDING
      );
      // Draw every sub stat
      ctx.font = `${FONT_SEMIBOLD} ${FONT_SIZE_SMALLEST} ${FONT_FAMILY}`;
      ctx.fillStyle = secondaryColor;
...
async function drawLightCone(ctx, lightConeData, primaryColor, secondaryColor) {
  ...
  try {
    // Picture from URL
    const lightConeImage = await loadImage(
      `${ASSETS_BASE_URL}${lightConeData.lightcone_preview_url}`
    );
    // Resize to 112px
    ctx.drawImage(
      lightConeImage,
      RELIC_RIGHT_ITEM_TL_X + 8,
      RELIC_RIGHT_ITEM_TL_Y + 8,
      112,
      112
    );
    // Lightcone box tag
    ...
    ctx.fillStyle = primaryColor;
    ctx.fillText(
      `S${lightConeData.lightcone_rank}`,
      RELIC_RIGHT_TAG_TEXT_TL_X,
      RELIC_RIGHT_TAG_TEXT_TL_Y
    );
    ctx.textAlign = "left";
    ...
    ctx.fillStyle = primaryColor;
    ctx.fillText(
      `${shortenString(lightConeData.lightcone_name, 20)}`,
      RELIC_RIGHT_STAT_NAME_TL_X,
      RELIC_RIGHT_STAT_NAME_TL_Y
    );
    // Lightcone stats
    ...
    ctx.fillText(
      `Level`,
      RELIC_RIGHT_STAT_NAME_TL_X,
      RELIC_RIGHT_STAT_NAME_TL_Y + RELIC_STAT_LINE_HEIGHT
    );
    ctx.fillText(
      `HP`,
      RELIC_RIGHT_STAT_NAME_TL_X,
      RELIC_RIGHT_STAT_NAME_TL_Y + RELIC_STAT_LINE_HEIGHT * 2
    );
    ctx.fillText(
      `ATK`,
      RELIC_RIGHT_STAT_NAME_TL_X,
      RELIC_RIGHT_STAT_NAME_TL_Y + RELIC_STAT_LINE_HEIGHT * 3
    );
    ctx.fillText(
      `DEF`,
      RELIC_RIGHT_STAT_NAME_TL_X,
      RELIC_RIGHT_STAT_NAME_TL_Y + RELIC_STAT_LINE_HEIGHT * 4
    );
    // Lightcone stat values (right aligned)
    ctx.textAlign = "right";
    ctx.fillText(
      `${lightConeData.lightcone_level}`,
      RELIC_RIGHT_STAT_VALUE_TL_X,
      RELIC_RIGHT_STAT_VALUE_TL_Y + RELIC_STAT_LINE_HEIGHT
    );
    ctx.fillText(
      `${lightConeData.lightcone_hp}`,
      RELIC_RIGHT_STAT_VALUE_TL_X,
      RELIC_RIGHT_STAT_VALUE_TL_Y + RELIC_STAT_LINE_HEIGHT * 2
    );
    ctx.fillText(
      `${lightConeData.lightcone_atk}`,
      RELIC_RIGHT_STAT_VALUE_TL_X,
      RELIC_RIGHT_STAT_VALUE_TL_Y + RELIC_STAT_LINE_HEIGHT * 3
    );
    ctx.fillText(
      `${lightConeData.lightcone_def}`,
      RELIC_RIGHT_STAT_VALUE_TL_X,
      RELIC_RIGHT_STAT_VALUE_TL_Y + RELIC_STAT_LINE_HEIGHT * 4
    );
    ctx.textAlign = "left";
  } catch {
...
async function drawBottom(
  ...
  ctx.textAlign = "right";
  ctx.fillText(
    `Level: ${level}  |  Achievements: ${achievementCount}`,
    LEVEL_TL_X,
    LEVEL_TL_Y
  );
  ctx.textAlign = "left";
...
async function drawWatermark(ctx, secondaryColor) {
  ...
  ctx.fillText(
    "@yookers | discord.gg/chives",
    WATERMARK_TL_X,
    WATERMARK_TL_Y + WATERMARK_LINE_HEIGHT
  );
}
...
function extractCharacterData(character_jsonData) {
  ...
  return {
    ...
    character_preview_url,
    character_path_icon_url,
    character_element_icon_url,
  };
}
...
function extractCharacterSkillsData(character_jsonData) {
  ...
  return {
    character_Basic_icon_url,
    character_Basic_level,
    character_Skill_icon_url,
    character_Skill_level,
    character_Ultimate_icon_url,
    character_Ultimate_level,
    character_Talent_icon_url,
    character_Talent_level,
  };
}
...
function extractCharacterAttributesData(character_jsonData) {
  ...
  return {
    attributes_base_hp,
    attributes_base_hp_icon,
    attributes_base_atk,
    attributes_base_atk_icon,
    attributes_base_def,
    attributes_base_def_icon,
    attributes_base_spd,
    attributes_base_spd_icon,
    attributes_base_crit_rate,
    attributes_base_crit_rate_icon,
    attributes_base_crit_dmg,
    attributes_base_crit_dmg_icon,
  };
}
...
async function cropImage(
  ...
  scaledCtx.drawImage(
    image,
    0,
    0, // Draw the image at 0,0 on the canvas
    image.width,
    image.height, // Original image size
    0,
    0, // Place it at 0,0 on the new canvas
    scaledWidth,
    scaledHeight // Resize image size on the new canvas
  );
  ...
  croppedCtx.drawImage(
    scaledCanvas,
    xOffset,
    yOffset, // Start cropping from this point on the scaled image
    targetWidth,
    targetHeight, // Crop size
    0,
    0, // Place the cropped part at 0,0 in the final image
    targetWidth,
    targetHeight // Resized image
  );
...
async function colorImage(imageURL, color) {
  ...
  coloringCtx.fillRect(
    0,
    0,
    coloringCtx.canvas.width,
    coloringCtx.canvas.height
  );
...
function sortAdditions(additions) {
  ...
  for (let addition of additions) {
    ...
    // For all other attributes, just assign the value.
    else {
...
async function checkColorCache(characterID, imageURL) {
  let colorsData = JSON.parse(
    fs.readFileSync(join(__dirname, "default-colors.json"))
  );
  if (!colorsData[characterID]) {
    const { primaryColor, secondaryColor } = await generateColorPalette(
      imageURL
    );
    ...
    fs.writeFileSync(
      join(__dirname, "default-colors.json"),
...
async function drawProfile(
  ...
  showWatermark,
  showUID,
  localDebugToggle = false
) {
  // Get JSON data from API
  const response = await fetch(
    `${API_BASE_URL_PREPEND}${UID}${API_BASE_URL_APPEND}`
  );
  ...
  const characterAttributesData =
    extractCharacterAttributesData(characterSelection);
  const characterAdditionsData = sortAdditions(
    extractCharacterAdditionsData(characterSelection)
  );
  ...
  if (!primaryColor && !secondaryColor) {
    ...
    const palette = await generateColorPalette(
      `${ASSETS_BASE_URL}${characterData.character_icon_url}`
    );
  ...
  await drawBottom(
    ctx,
    playerData.player_uid,
    playerData.player_level,
    playerData.player_achievement_count,
    primaryColor,
    showUID
  );
...
app.get("/api/generate", async (req, res) => {
  ...
  try {
    ...
    const imagePath = await drawProfile(
      ctx,
      canvas,
      uid,
      characterIndex,
